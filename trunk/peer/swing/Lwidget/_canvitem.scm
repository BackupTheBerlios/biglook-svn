...
(define-method (%canvas-item-draw ci::%canvas-line g::%awt-graphics)
   (with-access::%canvas-line ci (%shapes %fill-color %stroke %visible
					  %arrow-beg %arrow-end %arrow-path)
      (cond
	 ((%awt-graphics2D? g)
	  (if %visible
	      (begin
...
(define (%canvas-line-arrow o::%bglk-object)
   (with-access::%bglk-object o (%peer)
      (with-access::%canvas-line %peer (%arrow-beg %arrow-end)
	 (if %arrow-beg
	  (if %arrow-end '<-> '<-)
	  (if %arrow-end '-> '-)))))
...
(define (%canvas-line-arrow-shape-set! o::%bglk-object v)
   (match-case v
      ((?a ?b ?c)
       (with-access::%bglk-object o (%peer)
	  (with-access::%canvas-line %peer (%shapes %arrow-path
						    %arrow-beg %arrow-end)
...
(define (%canvas-line-cap-style-set! o::%bglk-object v)
   (with-access::%bglk-object o (%peer)
	 ...
	 (let ((width (%awt-basicstroke-line-width %stroke))
	       (join (%awt-basicstroke-line-join %stroke))
	       (dash (%awt-basicstroke-dash-array %stroke))
	       (cap (case v 
		       ((not-last) %awt-basicstroke-CAP_SQUARE)
		       ((butt) %awt-basicstroke-CAP_BUTT)
		       ((round) %awt-basicstroke-CAP_ROUND)
		       (else
			(error "cap-style-set!"
			       "Illegal style value" v)))))
	    (set! %stroke (%awt-basicstroke-new-full
			   width cap join 10.0 dash 0.0))))))
