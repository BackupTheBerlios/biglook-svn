...
(define (%event-x::int ev::%event)
   (let ((evt (%event-%event ev)))
	 ...
	 ((%swing-listselectionevent? evt)
	  -1)
	 ((%swing-tablemodelevent? evt)
	  (%swing-tablemodelevent-column evt))
	 (else
	  -1))))
...
(define (%event-mouse-click-count::int ev::%event)
   (%awt-mouseevent-click-count (%event-%event ev)))
