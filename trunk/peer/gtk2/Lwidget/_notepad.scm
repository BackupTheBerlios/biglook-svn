...
(define (%make-%notepad o::%bglk-object)
   (let ((notepad (gtkobject (%%gtk-notebook-new))))
      ;; Gtk configuration
      (%bglk-g-property-set! (gobject notepad) "tab-border" 0)
      (%bglk-g-property-set! (gobject notepad) "tab-hborder" 8)
      (%bglk-g-property-set! (gobject notepad) "tab_vborder" 2)
      (%bglk-g-property-set! (gobject notepad) "scrollable" #t)
      (instantiate::%notepad
	 (%bglk-object o)
	 (%builtin notepad))))
...
(define (%notepad-position o::%bglk-object)
   (gtk-position->biglook (g-property-get o "tab-pos")))

;*---------------------------------------------------------------------*/
;*    %notepad-position-set! ...                                       */
;*---------------------------------------------------------------------*/
(define (%notepad-position-set! o::%bglk-object v)
   (g-property-type-set! o "tab-pos" (biglook-position->gtk v) GTK-TYPE-POSITION))
...
(define (%notepad-selected-page o::%bglk-object)
   (with-access::%bglk-object o (%peer)
	 ...
	 (let* ((num (g-property-get o "page"))
		(page (%%gtk-notebook-get-nth-page (gtknotebook %builtin)
						   num)))
	    (%bglk-g-property-get (gobject (gtkobject page)) "user-data")))))
