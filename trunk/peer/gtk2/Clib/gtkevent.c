...
obj_t
bglk_g_object_get_user_data (GObject *gobj)
{
  GValue gval;
  memset (&gval, 0, sizeof (GValue));
  g_value_init (&gval, G_TYPE_POINTER);
  g_object_get_property (gobj, "user-data", &gval);
  return BREF (g_value_get_pointer (&gval));
}
...
/*---------------------------------------------------------------------*/
/*    int                                                              */
/*    bglk_event_click_count ...                                        */
/*---------------------------------------------------------------------*/
int
bglk_event_click_count( GdkEvent *ev ) {
   if( !ev ) {
      return -1;
   }
   switch( ((GdkEventAny *)ev)->type ) {
...
/*    bglk_event_callback ...                                          */
/*---------------------------------------------------------------------*/
static gboolean bglk_event_callback( GtkWidget* w, GdkEvent *ev, gpointer p ) {
   obj_t proc = (obj_t)p;
   obj_t bobj = bglk_g_object_get_user_data( G_OBJECT( w ) );
...
static void bglk_child_callback( GtkWidget* w, GtkWidget *c, gpointer p ) {
   obj_t proc = (obj_t)p;
   obj_t bobj = bglk_g_object_get_user_data( G_OBJECT( c ) );
...
bglk_install_widget_event_handler( GtkObject *o, obj_t p, char *name ) {
  g_signal_connect( o, name, G_CALLBACK( bglk_event_callback ), p );
}
