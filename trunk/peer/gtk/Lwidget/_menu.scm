...
(define (%make-%menu-radio o::%bglk-object texts::pair)
   (let loop ((tts texts)
	  ...
	  (let* ((text (car tts))
	     ...
	     (%bglk-gtk-arg-type-set! %radio "user_data" o GTK-TYPE-POINTER)
...
(define (%menubar-shadow::symbol o::%bglk-object)
   (gtk-shadow->biglook (gtk-arg-get o "shadow")))

;*---------------------------------------------------------------------*/
;*    %menubar-shadow-set! ...                                         */
;*---------------------------------------------------------------------*/
(define (%menubar-shadow-set! o::%bglk-object v::symbol)
   (gtk-arg-type-set! o "shadow" (biglook-shadow->gtk v) GTK-TYPE-SHADOW-TYPE))
