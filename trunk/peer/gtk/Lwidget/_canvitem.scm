...
(module __biglook_%canvas-item
   ...
   (extern (macro %%gtk-type-gdk-line-style::int
	   ...
	   (macro %%bglk-gtk-arg-pixbuf-set!::obj (::gtkobject*
						   ::string
						   ::gdkpixbuf*)
...
(define (%canvas-figure-color-set! o::%bglk-object v)
   (with-access::%canvas-figure (%bglk-object-%peer o) (%fill-color)
      (set! %fill-color v)
      (gtk-arg-set! o "fill_color" (biglook-color->rgb-string v))))

;*---------------------------------------------------------------------*/
;*    %canvas-item-width ...                                           */
;*---------------------------------------------------------------------*/
(define (%canvas-item-width::int o::%bglk-object)
   (inexact->exact (gtk-arg-get o "width")))
   
;*---------------------------------------------------------------------*/
;*    %canvas-item-width-set! ...                                      */
;*---------------------------------------------------------------------*/
(define (%canvas-item-width-set! o::%bglk-object v::int)
   (gtk-arg-set! o "width" (fixnum->flonum v)))
...
(define (%canvas-geometry-outline o::%bglk-object)
   (if (%canvas-line? (%bglk-object-%peer o))
       #unspecified
       (rgb-string-color->biglook (gtk-arg-get o "outline_color"))))
   
;*---------------------------------------------------------------------*/
;*    %canvas-geometry-outline-set! ...                                */
;*---------------------------------------------------------------------*/
(define (%canvas-geometry-outline-set! o::%bglk-object v)
   (gtk-arg-set! o "outline_color" (biglook-color->rgb-string v)))
...
(define (%canvas-shape-x o::%bglk-object)
   (flonum->fixnum (gtk-arg-get o "x1")))
   
;*---------------------------------------------------------------------*/
;*    %canvas-shape-x-set! ...                                         */
;*---------------------------------------------------------------------*/
(define (%canvas-shape-x-set! o::%bglk-object v)
   (gtk-arg-set! o "x1" (fixnum->flonum v)))

;*---------------------------------------------------------------------*/
;*    %canvas-shape-y ...                                              */
;*---------------------------------------------------------------------*/
(define (%canvas-shape-y o::%bglk-object)
   (flonum->fixnum (gtk-arg-get o "y1")))
   
;*---------------------------------------------------------------------*/
;*    %canvas-shape-y-set! ...                                         */
;*---------------------------------------------------------------------*/
(define (%canvas-shape-y-set! o::%bglk-object v)
   (gtk-arg-set! o "y1" (fixnum->flonum v)))

;*---------------------------------------------------------------------*/
;*    %canvas-shape-width ...                                          */
;*---------------------------------------------------------------------*/
(define (%canvas-shape-width o::%bglk-object)
   (absfx (-fx (flonum->fixnum (gtk-arg-get o "x2"))
	       (flonum->fixnum (gtk-arg-get o "x1")))))
...
(define (%canvas-shape-width-set! o::%bglk-object v)
   (if (<=fx v 0)
       (gtk-arg-set! o "x2" (gtk-arg-get o "x1"))
       (gtk-arg-set! o "x2"		     
		     (-fl (+fl (gtk-arg-get o "x1") (fixnum->flonum v)) 1.))))

;*---------------------------------------------------------------------*/
;*    %canvas-shape-height ...                                         */
;*---------------------------------------------------------------------*/
(define (%canvas-shape-height o::%bglk-object)
   (absfx (-fx (flonum->fixnum (gtk-arg-get o "y2"))
	       (flonum->fixnum (gtk-arg-get o "y1")))))
...
(define (%canvas-shape-height-set! o::%bglk-object v)
   (if (<=fx v 0)
       (gtk-arg-set! o "y2" (gtk-arg-get o "y1"))
       (gtk-arg-set! o
		     "y2"
		     (-fl (+fl (gtk-arg-get o "y1") (fixnum->flonum v)) 1.))))
...
(define (%canvas-line-points o::%bglk-object)
   (let ((l (%bglk-gtk-arg-get (%peer-%builtin (%bglk-object-%peer o))
			       "points")))
      (if (not (pair? l))
	  '()
	  l)))
...
(define (%canvas-line-points-set! o::%bglk-object v)
   (gtk-arg-type-set! o "points" v GTK-TYPE-GNOME-CANVAS-POINTS))
...
(define (%canvas-line-thickness-set! o::%bglk-object v)
   (gtk-arg-set! o "width_units" (fixnum->flonum v)))
	 
;*---------------------------------------------------------------------*/
;*    %canvas-line-arrow ...                                           */
;*---------------------------------------------------------------------*/
(define (%canvas-line-arrow o::%bglk-object)
   (let ((first (gtk-arg-get o "first_arrowhead"))
	 (last (gtk-arg-get o "last_arrowhead")))
      (if first
	  (if last '<-> '<-)
	  (if last '-> '-))))
...
(define (%canvas-line-arrow-set! o::%bglk-object v)
   (case v
      ((<->)
       (gtk-arg-set! o "first_arrowhead" #t)
       (gtk-arg-set! o "last_arrowhead" #t))
      ((<-)
       (gtk-arg-set! o "first_arrowhead" #t)
       (gtk-arg-set! o "last_arrowhead" #f))
      ((->)
       (gtk-arg-set! o "first_arrowhead" #f)
       (gtk-arg-set! o "last_arrowhead" #t))
      (else
...
(define (%canvas-line-arrow-shape o::%bglk-object)
   (list (gtk-arg-get o "arrow_shape_a")
	 (gtk-arg-get o "arrow_shape_b")
	 (gtk-arg-get o "arrow_shape_c")))
...
(define (%canvas-line-arrow-shape-set! o::%bglk-object v)
   (match-case v
      ((?a ?b ?c)
       (gtk-arg-set! o "arrow_shape_a" (exact->inexact a))
       (gtk-arg-set! o "arrow_shape_b" (exact->inexact b))
       (gtk-arg-set! o "arrow_shape_c" (exact->inexact c)))
      (else
...
(define (%canvas-line-style o::%bglk-object)
   (let ((style (gtk-arg-get o "line_style")))
...
(define (%canvas-line-cap-style o::%bglk-object)
   (let ((style (gtk-arg-get o "cap_style")))
...
(define (%canvas-image-image-set! o::%bglk-object v)
   (with-access::%bglk-object o (%peer)
      (with-access::%canvas-image %peer (%image)
	 (set! %image v)
	 (%%bglk-gtk-arg-pixbuf-set! (%peer-%builtin %peer)
				     "pixbuf"
				     (%%bglk-image->pixbuf
...
(define (%canvas-widget-widget-set! o::%bglk-object v)
   (with-access::%canvas-widget (%bglk-object-%peer o) (%widget)
      (set! %widget v)
      (%%bglk-gtk-arg-gtkobject-set! (%peer-%builtin (%bglk-object-%peer o))
				     "widget"
				     (%peer-%builtin (%bglk-object-%peer v)))))
